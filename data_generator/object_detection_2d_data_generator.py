...
class DataGenerator:
    ...
    def parse_xml(self,
        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            for image_id in it:
                # Construct the image filename with different possible extensions
                possible_extensions = ['.jpg', '.JPG', '.jpeg', '.HEIC']
                filename = '{}'.format(image_id)
                
                # Find the correct extension for the current image file
                ...
                if actual_extension is not None:
                    filename_with_extension = filename + actual_extension
                    self.filenames.append(os.path.join(images_dir, filename_with_extension))
                else:
                    # Handle the case when the image file with any of the specified extensions is not found
                    print(f"Image file not found for {filename} in {images_dir}")
